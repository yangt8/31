...
def Song_str(s:Song)->str:
    """ Song to string with formating"""
    return "Song's track:{:2d}  Title: {:30s}  Length: {:3d}  Play Count: {:2d}".format(s.track, s.title, s.length, s.play_count)
def Album_str(A:Album)->str:
    """ Album to str"""
    empty_str = ''
    print("Album's ID: ", A.id, " ", "Artist: ", A.artist, " ", "Title: ", A.title, "Years:", A.year)
    for b in A.songs:
        empty_str += Song_str(b) + "\n"
    return empty_str
def Songdisplay_str(S:Songdisplay)->str:
    """ Makes s_disp into a str"""
    empty_str = ''
    for c in S:
        empty_str += "Artist: {:18s}  Album's title: {:25s}  year: {:4d}  Track: {:2d}  Song's title: {:30s}  Length: {:3d}  Play Count: {:2d}".format(c.artist, c.a_title, c.year, c.track, c.s_title, c.length, c.play_count)+'\n'
    return empty_str
c = top_n_played(MUSIC, 10)
print(Songdisplay_str(c))
print()
#E2
print('---E2---')
def number_of_tracks(a:Album):
    return len(a.songs)
...
def unplayed_songs(l:list)->list:
    """ Takes list of albums and returns list"""
    result=[]
    for a in l:
        l2=Album_to_Songdisplays(a)
        for song in l2:
...
#E5
print('---E5---')
def Song_listening_time(s:Song):
    count=0
    count = s.length*s.play_count
    return count

def Album_listening_time(a:Album):
    count=0
    for s in a.songs:
        count += Song_listening_time(s)
    return count
      
def favorite_album(l:list):
    l.sort(key=Album_listening_time)
    return l[-1]
print(Album_str(favorite_album(MUSIC)))    
print()
#E6
print('---E6---')
def top_n(l:'list of Album', n: int, F:'Function on Albums', Boolean: 'True or Flase'):
    if Boolean == True:
        Songlist = all_songs(l)
        Songlist.sort(key=F, reverse=True)
        return Songlist[:n]
    else:
        Songlist = all_songs(l)
        Songlist.sort(key=F)
...
#E7
print('---E7---')
def favorite_album2(l:'list of Album',F:'Function on Albums'):
    l.sort(key=F)
    return l[-1]
...
#E8
print('---E8---')
def collection_search(l:'List of Albums',key:str):
    result=[]
    Songlist = all_Songdisplays(l)
    for s in Songlist:
        if key in s.s_title:
            result.append(s)
        elif key in s.artist:
            result.append(s)
        elif key in s.a_title:
            result.append(s)
    return result  
print()
